...
if __name__ == "__main__":
    ...
    if all_data:
        ...
        if not os.path.exists('filtered_bids_data.csv'):
            ...
            common_ids = df_existing['bidNtceNo'].isin(filtered_df['bidNtceNo']).sum()
            new_ids = len(filtered_df) - common_ids
            print(f"Total fetched IDs: {len(filtered_df)}")
            print(f"Existing IDs: {len(df_existing)}")
            print(f"Common IDs: {common_ids}")
            print(f"New unique IDs: {new_ids}")
            
            merged_df = pd.concat([df_existing, filtered_df]).drop_duplicates(subset='bidNtceNo', keep='last')
